...
public class ProgressService : IProgressService
{
    ...
    public IObservable<NewProgressEventArgs> NewProgressObservable => _progressSubject.AsObservable();

    public void UpdateProgress(double pct, string status, string status2) => _progressSubject.OnNext(new NewProgressEventArgs(pct, status, status2));

    public void UpdateProgress(double pct) => _progressSubject.OnNext(new NewProgressEventArgs(pct, "", ""));
