...
namespace Sims1WidescreenPatcher.Far.Models
{
    public class CompositeImageJob : IJob
    {
        ...
        public void Extract()
        {
            ...
            Log.Debug("Composite {Background} with bytes to {Output}. Width: {Width}, Height: {Height}", Background, Output, Width, Height);
            using (var compositeImage = new MagickImage(Bytes))
            {
                using (var stream = Assembly.GetExecutingAssembly()
                    .GetManifestResourceStream($"Sims1WidescreenPatcher.Far.Resources.{Background}"))
